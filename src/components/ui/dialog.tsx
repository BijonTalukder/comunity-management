...
/**
 * The scrollable region of a dialog. `flex-auto` + `min-h-0` lets it shrink
 * (and scroll) once the content exceeds the dialog's height cap, while the
 * header and footer stay pinned. The negative inline margin pulls the scroll
 * container out to the dialog edges so the scrollbar sits flush.
 */
function DialogBody({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "thin-scrollbar -mx-4 min-h-0 flex-auto overflow-y-auto px-4 py-0.5",
        className
      )}
      {...props}
    />
